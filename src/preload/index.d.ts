...
import { ChatTurnPayload, DecompositionResult, EngineResponse, LedgerNote, Subtask, Prerequisite, UpcomingTask, UpcomingSubtask, ActivityStats } from '../common/types'
...
declare global {
  interface Window {
    ...
    api: {
      ...
      selectAndExtractDocument: (projectId: string, taskTitle: string) => Promise<{ fileName: string; error?: string } | null>;
      updateChecklist: (
        projectId: string,
        taskId: string,
        request: string,
        ...
        depth: string,
        model: string
      ) => Promise<{ success: boolean; message: string; subtasks?: Subtask[]; prerequisites?: Prerequisite[] }>;

      getProjects: () => Promise<any[]>;
      createProject: (name: string) => Promise<any>;

      saveProjectData: (projectId: string, data: any, notes?: LedgerNote[]) => Promise<{ success: boolean; error?: string }>;
      ...
      saveContextLedger: (projectId: string, contents: string) => Promise<{ success: boolean; error?: string }>;

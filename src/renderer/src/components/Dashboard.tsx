...
export function Dashboard({ onOpenProject, onStartNow }: DashboardProps) {
  ...
  const handleDeleteProject = async (
    ...
    if (
      !window.confirm(
        `Delete "${projectName}"? This removes all its tasks and context permanently.`
      )
    )
      return
            ...
            <div className="mt-4">
              {moodMatches.length === 0 ? (
                <p className="py-4 text-center text-sm text-fg-3">
                  No {difficulty} tasks right now — nice work.
                </p>
              ) : (
                  ...
                  <div className="mb-4 flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-fg-3">
                        {new Date(project.lastActive).toLocaleDateString(undefined, {
                          month: 'short',
                          day: 'numeric'
                        })}
                      </span>

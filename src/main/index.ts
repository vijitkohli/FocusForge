...
function resolvePython(): string {
  ...
  const venvPython = join(engineDir, 'env', isWin ? 'Scripts' : 'bin', isWin ? 'python.exe' : 'python');
...
function createWindow(): void {
  ...
  ipcMain.handle('save-project-data', (_event, id, data, notes) => fsManager.saveProjectData(id, data, notes));
  ...
  ipcMain.handle('delete-task', (_event, projectId, taskId) => fsManager.deleteTask(projectId, taskId));

  ipcMain.handle('read-context-ledger', (_event, projectId) => fsManager.readContextLedger(projectId));

  ipcMain.handle('save-context-ledger', (_event, projectId, contents) => fsManager.writeContextLedger(projectId, contents));

  ipcMain.handle('read-user-profile', () => fsManager.readUserProfile());

  ipcMain.handle('save-user-profile', (_event, contents: string) => fsManager.writeUserProfile(contents));
  ...
  ipcMain.handle('reset-all-data', async () => {
    const { response } = await dialog.showMessageBox({
      ...
      message: 'This will permanently delete every project, task, context ledger, and your user profile. This cannot be undone.',
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('select-and-extract-document', async (_event, projectId: string, taskTitle: string) => {
    ...
    try {
      const result = await runPythonWithStdin('extract.py', { file_path: filePath, query: taskTitle })
  ...
  ipcMain.handle('update-checklist', async (_event, projectId: string, taskId: string, request: string, currentChecklist: { originalTask: string; subtasks: any[] }, deadline: string, depth: string, model: string) => {
    try {
      const [contextLedger, userProfile] = await Promise.all([
      ...
      if (task) {
        ...
        task.prerequisites = (response.data.prerequisites ?? []).map((p: any) => ({ ...p, isCompleted: false }))
      }

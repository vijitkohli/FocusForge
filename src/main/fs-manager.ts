...
import { LedgerNote, LedgerSection, UserProfileNote, UserProfileSection, UpcomingTask, UpcomingSubtask, ActivityStats } from '../common/types';
...
export class FileSystemManager{
    ...
    */
     ...
     */
    async deleteTask(projectId: string, taskId: string): Promise<{ success: boolean; error?: string }> {
        try {
     ...
     */
    async saveProjectData(projectId: string, data: any, notes?: LedgerNote[]): Promise<{ success: boolean; error?: string }> {
     ...
     */
    async writeContextLedger(projectId: string, contents: string): Promise<{ success: boolean; error?: string }> {
        const ledgerPath = path.join(ROOT_DIR, projectId, ledgerFileName(projectId));
        try {
    ...
    async appendLedgerNotes(projectId: string, notes: LedgerNote[]): Promise<void> {
        ...
        for (const { section, note } of notes) {
            ...
            contents = contents.slice(0, sectionEnd).replace(/\n*$/, '\n') + `${line}\n` + contents.slice(sectionEnd);
        }
    ...
    async writeUserProfile(contents: string): Promise<{ success: boolean; error?: string }> {
        ...
        if (missing.length > 0) {
            return { success: false, error: `Missing required section(s): ${missing.map((s) => `"## ${s}"`).join(', ')}` };
        }
        try {
    ...
    async readUserProfileForPrompt(): Promise<string> {
        ...
        for (const section of USER_PROFILE_TRIMMED) {
            ...
            if (bullets.length > MAX_USER_PROFILE_BULLETS) {
                ...
                contents = contents.slice(0, contentStart) + '\n' + trimmed + '\n' + contents.slice(sectionEnd);
            }
        }
        ...
        const overviewLines = projects.map((p) => {
            const daysSince = Math.floor((today - new Date(p.lastActive).getTime()) / (1000 * 60 * 60 * 24));
    ...
    private async updateProjectProgress(projectId: string, data: any): Promise<void> {
        ...
        const progress = allSubtasks.length === 0
            ? 0
            : Math.round((allSubtasks.filter((s: any) => s.isCompleted).length / allSubtasks.length) * 100);
        ...
        for (const project of projects) {
            ...
            for (const task of tasks) {
                ...
                for (const subtask of subtasks) {
                    ...
                    const daysRemaining = Math.round((subDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
